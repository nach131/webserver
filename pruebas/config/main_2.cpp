...
private:
    ...
    int apiPort;
    std::map<std::string, std::map<std::string, std::string> > locations; // Mapa de ubicaciones con sus configuraciones

public:
    ServerConfiguration() : port(8080), apiPort(3000), apiForward("192.168.1.20") {} // Valores por defecto
        ...
        while (std::getline(configFile, line))
        {
            if (line.find("server {") != std::string::npos)
            {
                while (std::getline(configFile, line))
                {
                    ...
                    {
                        location = value;
                        locations[location] = std::map<std::string, std::string>();
                        while (std::getline(configFile, line))
                        {
                            if (line.find("}") != std::string::npos)
                                break;
                            ...
                            {
                                locations[location][key2] += value2 + " ";
                            }
                            else
                            {
                                locations[location][key2] = value2;
