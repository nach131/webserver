...
int main()
{
    ...
    while (true)
    {
        ...
        server = gethostbyname("127.0.0.1/api/users");
        std::cout << "server: " << server << std::endl;

        if (server == NULL)
        {
            std::cerr << "Error al obtener el host: " << hstrerror(h_errno) << std::endl;
            close(newsockfd);
            continue;
        }
        ...
        memcpy(&forward_serverAddr.sin_addr.s_addr, server->h_addr, server->h_length);
